...
static bool mergeConcatChildren(NodeOp op, std::vector<Node *> & children, NodeOp * newOp) {
    ...
    for (const auto & child : children) {
        if (child->op == CONCAT) {
            numConcatChildren += 1;
            if (firstConcatChild == NULL) {
                firstConcatChild = child;
            }
            else {
                if (child->children->size() != firstConcatChild->children->size()) {
                    mergeConcat = false;
                    break;
                }
                for (int32_t i = 0; i < (int32_t) child->children->size(); i += 1) {
                    if (child->children->at(i)->width != firstConcatChild->children->at(i)->width) {
                        mergeConcat = false;
                        break;
                    }
                }
    ...
    if (mergeConcat && numConcatChildren > 1) {
        std::vector<Node *> newChildren;
        std::vector<std::vector<Node *>> xorsChildren(firstConcatChild->children->size(), std::vector<Node *>(0));
        for (const auto & c : children) {
            if (c->op == CONCAT) {
                for (int32_t i = 0; i < (int32_t) c->children->size(); i += 1) {
                    xorsChildren[i].push_back(c->children->at(i));
                }
            }
            else {
        ...
        std::vector<Node *> concatChildren;
        for (const auto & xorChildren : xorsChildren) {
            Node & opNode = simplify(Node::OpNode(op, xorChildren));
            concatChildren.push_back(&opNode);
        }
        if (newChildren.size() == 0) {
            *newOp = CONCAT;
...
Node & simplifyCore(Node & node, bool propagateExtractInwards, bool useSingleBitVariables) {
    ...
    if (op == BXOR or op == BAND or op == BOR) {
        ...
        if (allChildrenZeroExt || allChildrenSignExt) {
            NodeOp extType = newChildren[0]->op;
            Node * extWidthNode = newChildren[0]->children->at(0);

            std::vector<Node *> childrenList;
            for (const auto & child : newChildren) {
                childrenList.push_back(child->children->at(1));
            }
            Node & opNode = simplify(Node::OpNode(op, childrenList));
                ...
                if (child0.op == op1) {
                    ...
                    while (j < (int32_t) child0.children->size()) {
                        ...
                        while (k < (int32_t) newChildren.size()) {
                            ...
                            if ((child1.op == BNOT && equivalence(*child1.children->at(0), *child0.children->at(j))) || (child0.children->at(j)->op == BNOT && equivalence(child1, *child0.children->at(j)->children->at(0)))) {
