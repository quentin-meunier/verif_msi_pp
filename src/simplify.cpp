...
static void bitDecomposeCst(Node & node) {
    if (node.extractBit != NULL) {
        return;
    }
    node.extractBit = new Node * [node.width];
    ...
    for (int32_t limb = 0; limb < node.nlimbs; limb += 1) {
        ...
        for (int32_t b = 0; b < limit; b += 1) {
            if (((node.cst[limb] >> b) & 1) == 0) {
                node.extractBit[idx] = &cst0;
            }
            else {
                node.extractBit[idx] = &cst1;
            }
            idx += 1;
        }
    }
    ...
    if (op == BXOR) {
        ...
        if (isZero(sh0.cst, sh0.width)) {
            ...
            if (exp.op == LSHL) {
                ...
                bool singleBitOrCstChildren = true;
                for (const auto & gChild : *exp.children) {
                    if (gChild->width != 1 and gChild->nature != CONST) {
                        singleBitOrCstChildren = false;
                        break;
                    }
                }
                if (singleBitOrCstChildren) {
                    std::vector<Node *> singleBitChildren;
                    for (const auto & gChild : *exp.children) {
                        if (gChild->nature == CONST) {
                            bitDecomposeCst(*gChild);
                            for (int32_t i = 0; i < gChild->width; i += 1) {
                                singleBitChildren.push_back(gChild->extractBit[i]);
                            }
                        }
                        else {
                            singleBitChildren.push_back(gChild);
                        }
                    }
                    ...
                    for (int32_t i = 0; i < (int32_t) singleBitChildren.size() - (int32_t) sh0.cst[0]; i += 1) {
                        newChildrenShifted.push_back(singleBitChildren[i]);
                    }
                    // Merging adjacent constants
                    std::vector<Node *> newChildrenMerged;
                    int i = 0;
                    std::vector<Node *> v;
                    int32_t size = (int32_t) newChildrenShifted.size();
                    while (i < size) {
                        while (i != size and newChildrenShifted[i]->nature == CONST) {
                            v.push_back(newChildrenShifted[i]);
                            i += 1;
                        }
                        if (v.size() != 0) {
                            std::reverse(v.begin(), v.end());
                            newChildrenMerged.push_back(&Concat(v));
                            v.clear();
                        }
                        if (i == size) {
                            break;
                        }
                        newChildrenMerged.push_back(newChildrenShifted[i]);
                        i += 1;
                    }
                    return setSimpEqAndReturn(node, Node::OpNode(CONCAT, newChildrenMerged));
                }
            }
            return setSimpEqAndReturn(node, defaultNode(node, op, newChildren, modified));
        }
        ...
        if (isZero(sh0.cst, sh0.width)) {
            ...
            if (exp.op == ASHR) {
                ...
                if (singleBitOrCstChildren) {
                    ...
                    std::vector<Node *> newChildrenShifted;
                    for (int32_t i = (int32_t) sh0.cst[0]; i < (int32_t) singleBitChildren.size(); i += 1) {
                        newChildrenShifted.push_back(singleBitChildren[i]);
                    }
                    for (int32_t i = 0; i < (int32_t) sh0.cst[0]; i += 1) {
                        newChildrenShifted.push_back(singleBitChildren[singleBitChildren.size() - 1]);
                    }
                    // Merging adjacent constants
                    std::vector<Node *> newChildrenMerged;
                    int i = 0;
