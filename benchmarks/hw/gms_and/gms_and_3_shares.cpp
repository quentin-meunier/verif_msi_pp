...
void usage(const char * argv) {
    ...
    std::cout << "   TThis script contains a VerifMSI++ description of a circuit implementing the logical AND following the GMS scheme with 3 shares from [1]." << std::endl;
...
int main(int argc, const char ** argv) {
    ...
    if (order >= 3) {
        std::cerr << "*** Error: the order of verification (3) must be less than the number of shares (2)" << std::endl;
        exit(1);
    } 
